...
#if defined(ASTERIUS)
decodeLatin1 bs =
  case fromJSArrayBuffer
         (jsStringEncodeUTF16LE
            (fromJSArrayBuffer
               (jsStringEncodeUTF16LE
                  (jsStringDecodeLatin1 (byteStringToJSArrayBuffer bs))))) of
    ba -> text (A.Array ba) 0 (I# ((sizeofByteArray# ba) `quotInt#` 2#))
#else
...
decodeUtf8With _ bs =
  case fromJSArrayBuffer
         (jsStringEncodeUTF16LE
            (jsStringDecodeUTF8 (byteStringToJSArrayBuffer bs))) of
    ba -> text (A.Array ba) 0 (I# ((sizeofByteArray# ba) `quotInt#` 2#))
#else
...
#if defined(ASTERIUS)
encodeUtf8 (Text (A.Array ba) (I# off) len) =
  byteStringFromJSArrayBuffer
    (jsStringEncodeUTF8
       (jsStringDecodeUTF16LE
          (toJSArrayBuffer
             ((byteArrayContents# ba) `plusAddr#` (off *# 2#))
             (len * 2))))
#else
